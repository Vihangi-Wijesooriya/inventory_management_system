...
$countStmt = db()->prepare("SELECT COUNT(*) FROM suppliers $whereSql");
$countStmt->execute($params);
$total = (int)$countStmt->fetchColumn();
$totalPages = max(1, (int)ceil($total / $perPage));

$sql = "SELECT * FROM suppliers $whereSql ORDER BY name LIMIT $perPage OFFSET $offset";
$stmt = db()->prepare($sql);
$stmt->execute($params);
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="<?= url('modules/suppliers/create.php') ?>" class="btn btn-primary btn-sm">
                ...
                <?php foreach ($suppliers as $s): ?>
                    <tr>
                        <td><strong><?= e($s['name']) ?></strong>
                            <?php if ($s['address']): ?><div class="small text-muted"><?= e($s['address']) ?></div><?php endif; ?>
                        </td>
                        ...
                        <td class="text-end">
                            <a href="<?= url('modules/suppliers/edit.php?id=' . (int)$s['supplier_id']) ?>" class="btn btn-sm btn-outline-primary">
                            ...
                            <?php if (is_admin()): ?>
                                <form method="POST" action="<?= url('modules/suppliers/delete.php') ?>" class="d-inline"
                                    ...
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="id" value="<?= (int)$s['supplier_id'] ?>">
